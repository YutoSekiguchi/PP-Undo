...
func InitRouter(db *gorm.DB) {
	...
	noteFolder := e.Group("/notefolders")
	{
		...
		noteFolder.PUT("", ctrl.HandleUpdateNoteFolderTitleByID)
	...
	strokes := e.Group("/strokes")
	{
		...
		strokes.GET("/group/:id1/:id2/:gnum", ctrl.HandleUpdateStrokesGroup)
		strokes.GET("/update/pressures", ctrl.HandleUpdateTransformPressures)
